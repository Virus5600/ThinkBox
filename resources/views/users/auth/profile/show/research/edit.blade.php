...
<div class="container-fluid my-5 px-5">
	<div class="row">
		<div class="col-12 my-3">
			<form action="{{route('profile.research.update', [$research->id])}}" method="POST" enctype="multipart/form-data">
				{{csrf_field()}}

				<div class="row">
					<div class="form-group col-lg-6">
						<label class="form-label font-weight-bold important" for="title">Research Title</label>
				...
				<div class="row">
						...
						<div class="row">
							<div class="form-group col-12 col-lg-6">
								...
								<select class="selectpicker border rounded multiple-select w-100" name="registeredAuthors[]" data-live-search="true" multiple>
									@foreach ($staff as $s)
									<option value="{{$s->id}}" {{($research->researchAuthors->contains('staff_id', $s->id)) ? 'selected' : ''}}>{{$s->user->getFullName()}}</option>
									@endforeach
								</select>
					...
					<div class="form-group col-12 col-lg-6">
						...
						<input type="date" class="form-control" name="date_published" value="{{\Carbon\Carbon::parse($research->date_published)->format('Y-m-d')}}"/>
				...
				<div class="row">
					<div class="form-group col-12">
						...
						<textarea class="form-control not-resizable" rows="5" name="description" placeholder="Provide an abstract or description here...">{{$research->description}}</textarea>
				...
				<div class="row">
					<div class="col col-lg-6 form-group">
						<label class="form-label font-weight-bold">Research Focus</label><br>
						<select class="selectpicker border rounded multiple-select" name="focus[]" data-live-search="true" multiple>
							@foreach ($focus as $f)
							<option value="{{$f->id}}" {{$research->researchFocus->contains('id', $f->id) ? 'selected' : ''}}>{{ucwords($f->name)}}</option>
							@endforeach
						</select>
				...
				<div class="row">
					...
					<div class="form-group col-12 d-flex flex-d-row" id="misc">
						<div class="form-check mx-3">
							<input class="form-check-input" type="checkbox" name="is_file_requestable" id="is_file_requestable" {{$research->is_file_requestable ? 'checked' : ''}}/>
						...
						<div class="form-check mx-3">
							<input class="form-check-input" type="checkbox" name="is_featured" id="is_featured" {{$research->is_featured ? 'checked' : ''}}/>
